...
describe('AssetModel', () => {
  ...
  const assetData = XhrMock.ASSET_FEATURED.data;

  /**
   * @type AssetModel
   */
  const assetModel = ClassMock.build(AssetModel, assetData.id, XhrMock.EVENT_ID_FEATURED);
  ...
  it('can be marked as dead', () => {
    ...
    expect(FetchShim.fetch).toHaveBeenCalledWith(`${ClassMock.sdk.apiBaseUrl}/events/${XhrMock.EVENT_ID_FEATURED}/assets/${assetData.id}/check`, { method: 'PATCH' });
  });
  ...
  it('can be reported', () => {
    ...
    expect(FetchShim.fetch).toHaveBeenCalledWith(`${ClassMock.sdk.apiBaseUrl}/events/${XhrMock.EVENT_ID_FEATURED}/assets/${assetData.id}/report`, { method: 'PATCH' });
  });
});
